  ...
  implements ByteFieldCodec
{
  ...
  public DecResult decode(DecContext ctx)
  {
    ...
    {
      String errmsg = "ShortCodec: not enough bytes for decode, need [" + byteLength + "], actually [" + bytes.length + "].";
    ...
    return new DecResult(Short.valueOf(numberCodec.bytes2Short(bytes, byteLength)), ArrayUtils.subarray(bytes, byteLength, bytes.length));
  }
